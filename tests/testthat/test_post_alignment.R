...
test_that("test_setAlignmentRank", {
  ...
  setAlignmentRank(df, ref = "run1", eXp = "run2", tAligned, XICs.eXp, params, adaptiveRT)
  expect_equal(c(NA_integer_, 1L, 1L), df[,alignment_rank])

  # 2nd case
  df <- data.table::data.table(multipeptide_DIAlignR[["14383"]])
  df$alignment_rank[1] <- 1L; df$m_score[3] <- 0.03
  setAlignmentRank(df, ref = "run1", eXp = "run2", tAligned, XICs.eXp, params, adaptiveRT)
  expect_equal(c(1L, NA_integer_, NA_integer_), df[,alignment_rank])

  # case 3
  setAlignmentRank(df, ref = "run0", eXp = "run2", tAligned, XICs.eXp, params, adaptiveRT)
  expect_equal(c(1L, NA_integer_, 1L), df[,alignment_rank])

  # case 4
  df <- data.table::data.table(multipeptide_DIAlignR[["14383"]])
  df$alignment_rank[2] <- 1L; df$m_score[3] <- 0.06
  setAlignmentRank(df, ref = "run1", eXp = "run2", tAligned, XICs.eXp, params, adaptiveRT)
  expect_equal(df[,alignment_rank], c(NA_integer_, 1L, NA_integer_))
...
test_that("test_setOtherPrecursors", {
  ...
  setOtherPrecursors(df, "run2", XICs.eXp, analytes = 4618L, params)
  expect_equal(df[,alignment_rank], rep(NA_integer_, 3))
  ...
  setOtherPrecursors(df, "run2", XICs.eXp, analytes = c(9719L, 9720L), params)
  ...
  df <- data.table::data.table(multipeptide_DIAlignR[["9861"]])
  setOtherPrecursors(df, "run2", XICs.eXp, analytes = c(9719L, 9720L), params)
  expect_equal(df[,alignment_rank], rep(NA_integer_, 4))

  df[3, alignment_rank := 1L]
  setOtherPrecursors(df, "run1", XICs.eXp, analytes = c(9719L, 9720L), params)
  expect_equal(df[,alignment_rank], c(NA_integer_, 1L, 1L, NA_integer_))
})
...
test_that("test_reIntensity", {
  ...
  expect_equal(df[3,intensity], 255.496)

  df[3, alignment_rank:= 1L]
  reIntensity(df, "run2", XICs.eXp, params)
  expect_equal(df[3, intensity], 211.3709, tolerance = 1e-05)
  ...
  df$alignment_rank[3:4] <- 1L
  reIntensity(df, "run2", XICs.eXp, params)
  expect_equal(df[3, intensity], 52.95950)
  expect_equal(df[4, intensity], 24.50702, tolerance = 1e-05)
  ...
  expect_equal(df[3, intensity], 20.11727)
})
